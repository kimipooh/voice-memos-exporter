...
<p align="center">
  <img src="app_icon.png" alt="Voice Memos Exporter アイコン" width="128" height="128">
</p>

これは [rudrakabir/voice-memos-exporter](https://github.com/rudrakabir/voice-memos-exporter)（作者: [rudrakabir](https://github.com/rudrakabir)）の開発中のforkです。macOSのボイスメモを一括書き出しできる有用なツールを作った原作者に感謝します。このforkでは、書き出しの信頼性を改善し、PythonのコマンドラインとGUIを正式インターフェースとして提供します。

upstreamのライセンスは現在不明確です。このforkを独立したReleaseやバイナリ配布物として提供していません。詳しくは[ライセンス](#ライセンス)を参照してください。

<p align="center">
  <img src="screenshot.png" alt="Voice Memos Exporter スクリーンショット" width="800">
</p>
...
- GUIのコピー処理をワーカースレッドで実行し、進捗表示と**Cancel**を提供します。
- 書き出し時に録音の `ZDATE` を使い、更新日時（`mtime`）とアクセス日時（`atime`）を明示的に設定します（upstream Issue #1への部分対応）。
- 共有ロジックを `vmx_core.py` に置き、core、CLI、GUIの選択動作に回帰テストを追加しました。
- PyInstaller specで `target_arch='universal2'` を指定しました（upstream Issue #4）が、このビルドは未検証です。
- CLIとGUIの両方で録音件数を表示します（upstream Issue #3）。

## 要件と互換性

- ボイスメモのデータが `~/Library/Group Containers/group.com.apple.VoiceMemos.shared/` にあるmacOS。オプションのapp用specが宣言する最小バージョンはmacOS 12.0です。
- Python 3と標準ライブラリ。実行時に `requirements.txt` のパッケージは不要で、記載されている `pyinstaller` はオプションのappをビルドするときだけ使います。
- ボイスメモデータベースを読むプロセスへのFull Disk Access。

### Pythonソースの互換性

ソースはPython標準ライブラリだけを使い、アーキテクチャ依存コードを含みません。Apple Siliconで動作確認済みです。同じソースはIntel Macでも動作する見込みですが、Intel実機では検証していません。

### ビルド済みappの互換性

upstream配布のappはApple Silicon専用です（upstream Issue #4）。このforkではPythonソースから直接実行できる経路を用意して、この制約に対応しました。`.spec` は `target_arch='universal2'` を指定しますが、Universal 2ビルドは未検証です。ビルドにはpython.orgのmacOS universal2 installerなどのUniversal 2 Pythonが必要です。単一アーキテクチャのHomebrew Pythonではこのbundleを作れません。

## Full Disk Access

ボイスメモのデータはmacOSのTCCで保護されており、Full Disk Accessが必要です。権限は、実際にデータベースへアクセスするプロセスに与えます。

- Python CLIまたはGUIをソースから起動する場合は、Terminal.app、iTerm2、統合ターミナルを提供するVS Codeなど、Pythonを起動するターミナルアプリに権限を与えます。`python3` バイナリ単体を追加するのではありません。
- パッケージ化した `.app` を使う場合は、`.app` 自体に権限を与えます。

**システム設定 → プライバシーとセキュリティ → フルディスクアクセス**を開き、対象アプリを追加して有効にした後、変更を反映するために**そのアプリを再起動**してください。
...
既存のTkinter GUIも正式サポート対象として維持しています。

```bash
python3 voice_memos_exporter.py
```

検索、フィルタ後も維持される安定した選択、Select All/Deselect All、進捗、キャンセル、録音単位の結果処理に対応します。GUIとCLIは同じ `vmx_core.py` の書き出し実装を呼び出します。
...
`vmx_core.py` は信頼性の中心で、データベースの診断と読み込み、ソース解決、安全な出力先生成、書き出し、診断ログを担当します。CLIとGUIは、その共通動作の表示層です。
...
PyInstaller specは、補助的なパッケージ方法として残しています。
...
specはUniversal 2を指定します。Pythonのアーキテクチャ、Apple Silicon/Intelの対象、コード署名、公証、Gatekeeper、Full Disk Accessを与えるプロセスの違いにより、appのビルドと実行は環境依存です。このforkの文書作業では `.app` ビルドを必須とせず、検証もしていません。forkのReleaseもありません。
...
2026-08-27（macOS 26、Apple Silicon）の実測結果:

| Python | 結果 |
|---|---|
| `/usr/bin/python3`（Tkinterあり） | 49件、**OK**、スキップなし |
| Homebrew `python@3.12`（`_tkinter` なし） | 49件、**OK (skipped=1)** |

スキップされる1件はGUI選択テストです。Tkinterが使えない環境でスキップします。原因は、Pythonビルドに `_tkinter` モジュールが含まれない場合（Homebrew Pythonは含みません）と、ディスプレイへ接続できない場合の2通りです。どちらの場合もGUI以外のテストはすべて実行されます。

## 既知の制限

- upstreamのライセンスが未解決のため、このforkにはReleaseもバイナリ配布もありません。
- Intel Macでのソース実行とUniversal 2 appビルドは未検証です。
...
- 将来macOSのボイスメモスキーマが変わると、現在のreaderでは対応できなくなる可能性があります。
