...
class VoiceMemosExporter:
    ...
    def _display_values(recording):
        ...
        if recording.duration is None:
            ...
            seconds = max(0, int(recording.duration))
    ...
    def filter_recordings(self, *args):
        ...
        for key, recording in self.recordings.items():
            display_values = self._display_values(recording)
            if not search_term or any(search_term in value.lower() for value in display_values):
                visible.append(key)
        self._render_keys(visible)
